...
int main()
{
	...
	do
	{
		...
		switch(choice)
		{
			...
			case 2:		if(instant.isEmpty() == false) //check if there's anyone in the instant queue---------------------------------------------
						{
							//remove the front of the instant queue-----------------------------------------
							name = instant.deQueue();
							cout << "\t\t" << name << ", InstantPass holder, YOU ARE RIDING!\n";
						}
						else if(normal.isEmpty() == false) //check if there's anyone in the normal queue---------------------------------
						{
